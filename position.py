...
def determine_position(bbox1, bbox2, relation, iou_thresh, distance_thresh):
    ...
    试一试老师说的
    '''
    # 不确定是什么坐标系，先假设是左上角坐标系
    # bbox1是修改之后的，bbox2是原来的
...
def determine_position_multi_bbox(bbox1, bbox2, relation, iou_thresh, distance_thresh):
    if len(bbox1) == 1:
        return int(determine_position(bbox1[0], bbox2, relation, iou_thresh, distance_thresh))
    else:
        flag1 = all([determine_position(b1, bbox2, relation, iou_thresh, distance_thresh) for b1 in bbox1])
        flag2 = any([determine_position(b1, bbox2, relation, iou_thresh, distance_thresh) for b1 in bbox1])
        if flag2:
            if flag1:
                return 1
            else:
...
if __name__ == "__main__":
    ...
    for item in tqdm(user_data):
        for image_idx in idx2model:
            ...
            if "None" in entry['edited_image_edited_area'] or "None" in entry['original_image_edited_area']:
                position_score = 1
                ...
                if entry['original_image_edited_area'] == 'None':
                    source_text_prompt = entry['being_added']
                # load image
                ...
                if entry['original_image_edited_area'] == 'None':
                    edited_text_prompt = entry['added_object']
                # load image
                ...
                if len(source_boxes_filt) == 0:
                    ...
                    if len(edited_boxes_filt) == 0:
                        ...
                        position_score = determine_position_multi_bbox(source_boxes_filt, edited_boxes_filt, entry['relation']).item()
            
            # save data
            item[f'edited_image{image_idx}'][f'my_position'] = float(position_score)
