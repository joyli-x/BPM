...
if __name__ == "__main__":
    ...
    for i, box in enumerate(boxes_filt):
        if pred_phrases[i].startswith("cat"):
            show_mask(masks[i].cpu().numpy(), plt.gca(), random_color=True)
            show_box(box.numpy(), plt.gca(), pred_phrases[i])
    # for mask in masks:
    ...
    plt.axis('off')
    plt.savefig(
        os.path.join(output_dir, "cat_real.jpg"), 
